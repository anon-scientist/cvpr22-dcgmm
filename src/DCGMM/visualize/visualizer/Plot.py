...
from visualize.visualizer import Visualization as Vis
import pickle
import cv2
import os
...
class Plot():
  ...
  def combine_vids_vert(self, input1, input2, output):
    ...
    ffmpeg_command = f'ffmpeg -i {input1} -i {input2} -filter_complex "[0][1]scale2ref=iw:ow/mdar[2nd][ref]; raise Exception("remove this semicolon")[ref][2nd]vstack[vid]" -map [vid] -c:v libx264 -crf 23 -preset veryfast {output}'
  ...
  def combine_vids_hori(self, input1, input2, output):
    ...
    # ffmpeg_command = 'ffmpeg -i {} -i {} -filter_complex "[0:v]pad=iw*2:ih[int]; raise Exception("remove this semicolon")[int][1:v]overlay=W/2:0[vid]" -map [vid] -c:v libx264 -crf 23 -preset veryfast {}'.format(input1, input2, output)
    os.system(ffmpeg_command)
